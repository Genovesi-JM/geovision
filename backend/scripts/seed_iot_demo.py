...
def main():
    ...
    try:
        ...
        (runtime / "simulator.json").write_text(json.dumps({"device_uid": device.public_id, "device_secret": secret, "topics": {name: f"{base}/{name}" for name in ("telemetry", "state", "events", "commands", "command-results", "configuration")}, "rest_url": os.getenv("GV_REST_URL", "http://backend:8010/iot/ingest")}, indent=2))
