...
def _ingest(client, uid, secret, message_id, temperature, safety=True):
    return client.post(
        "/iot/ingest",
        headers={"Authorization": f"Device {secret}", "X-Device-ID": uid},
        json={
...
def test_provision_rest_ingest_duplicate_alert_resolution_and_safe_command(
    client, db_session
):
    ...
    asset = client.post(
        "/iot/assets",
        headers=headers,
        json={"name": "Water tank A", "site_id": site.id, "asset_type": "tank"},
    )
    assert (
        asset.status_code == 201
        and client.get("/iot/assets", headers=headers).json()[0]["name"]
        == "Water tank A"
    )
    gateway = client.post(
        "/iot/gateways",
        headers=headers,
        json={"name": "Plant gateway", "site_id": site.id, "gateway_type": "edge"},
    )
    assert (
        gateway.status_code == 201
        and client.get("/iot/gateways", headers=headers).json()[0]["name"]
        == "Plant gateway"
    )
    ...
    assigned = client.post(
        f"/iot/alerts/{alerts[0]['id']}/assign",
        headers=headers,
    ...
    command = client.post(
        f"/iot/devices/{device_id}/commands",
        headers=headers,
        json={
            "name": "relay_on",
            "confirmed": True,
            "reason": "Supervised low-voltage test relay",
            "fail_safe_state": "off",
        },
    )
    ...
    result = client.post(
        "/iot/device/command-results",
        headers={"Authorization": f"Device {secret}", "X-Device-ID": uid},
        json={
            "command_id": command_id,
            "status": "completed",
            "actual_state": {"relay": True},
        },
    )
...
def test_signed_mqtt_ingestion_path(client, db_session):
    ...
    db_session.commit()
    company, site = _tenant(db_session, owner_email)
    headers = _auth(client, owner_email, "long-password-123")
    device, secret = _provision(client, headers, site.id)
    payload = {
        "device_uid": device["device_uid"],
        "message_id": f"mqtt-{uuid.uuid4().hex}",
    ...
    mqtt_bridge._process(
        company.id, site.id, device["device_uid"], "telemetry", payload
    )
    latest = client.get(f"/iot/devices/{device['id']}/latest", headers=headers)
    assert latest.status_code == 200
    assert any(
        row["channel"] == "temperature" and row["value"] == 22.2
        for row in latest.json()["readings"]
    )
...
def test_solution_kit_catalogue_and_one_step_provisioning(client, db_session):
    ...
    assert water["sectors"] == ["construction_infrastructure", "environment"]
    assert "industry" not in water
    standby_kit_ids = {
        "cold_chain_starter",
    ...
    assert standby_kit_ids.isdisjoint({kit["id"] for kit in kits})
    assert client.get("/iot/kits/does-not-exist", headers=headers).status_code == 404
    for kit_id in standby_kit_ids:
        assert client.get(f"/iot/kits/{kit_id}", headers=headers).status_code == 404
        standby = client.post(
            f"/iot/kits/{kit_id}/provision",
            headers=headers,
    ...
    wrong_sector = client.post(
        "/iot/kits/soil_control/provision",
        headers=headers,
        json={"site_id": site.id},
    )
    ...
    provisioned = client.post(
        "/iot/kits/water_tank_starter/provision",
        headers=headers,
        json={"site_id": site.id},
    )
    ...
    channels = {
        r["channel"]
        for r in client.get(f"/iot/devices/{device_id}", headers=headers).json()[
            "readings"
        ]
    }
    assert (
        "tank_level" not in channels
    )  # no readings yet, but channels exist for ingestion below
    ...
    alerts = client.get("/iot/alerts", headers=headers).json()
    assert any(
        a["channel"] == "tank_level" and a["severity"] == "critical" for a in alerts
    )
...
def test_gps_kit_reports_location_for_maps(client, db_session):
    ...
    provisioned = client.post(
        "/iot/kits/gps_asset_tracker/provision",
        headers=headers,
        json={"site_id": site.id},
    )
    assert provisioned.status_code == 201, provisioned.text
